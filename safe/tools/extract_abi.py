...
def replace_for_replay(value: str) -> str:
    ...
    def rewrite_historical_root(value: str) -> str | None:
        for marker, placeholder, known_root in [
            ("build-check", "$SAFE_VENDOR_BUILD_CHECK", VENDOR_BUILD_CHECK),
            ("original", "$SAFE_VENDOR_ORIGINAL", VENDOR_ORIGINAL),
        ]:
            needle = f"/{marker}/"
            if value == f"/{marker}" or value.endswith(f"/{marker}"):
                return placeholder
    ...
    def rewrite_embedded_roots(text: str) -> str:
        for src, dst in replacements.items():
            if src in text:
                text = text.replace(src, dst)
        pattern = re.compile(r"/[^\s\"']+/(?:build-check|original)(?:/[^\s\"']*)?")
        for match in list(pattern.finditer(text)):
            rewritten = rewrite_historical_root(normpath(match.group(0)))
            if rewritten is not None:
                text = text.replace(match.group(0), rewritten)
        return text
    ...
    rewritten_embedded = rewrite_embedded_roots(value)
    if rewritten_embedded != value:
        value = rewritten_embedded
        return value
    if value.startswith("-"):
        return value
    ...
    historical = rewrite_historical_root(value)
...
def build_ninja_link_objects() -> dict[str, list[str]]:
    ...
    for line in logical_lines:
        ...
        if objects:
            mapping[replace_for_replay(str(build_root / output))] = objects
    return mapping
...
def build_test_source_path_map() -> list[dict[str, object]]:
...
def build_patch_manifest() -> None:
    provenance_rows = {}
    provenance_doc = Path(__file__).resolve().parents[1] / "docs" / "debian-patch-provenance.md"
    for raw_line in provenance_doc.read_text().splitlines():
        line = raw_line.strip()
        if not line.startswith("|") or line.startswith("| Patch ") or line.startswith("| ---"):
            continue
        columns = [column.strip() for column in line.strip("|").split("|")]
        if len(columns) != 6:
            raise ValueError(f"Malformed debian patch provenance row: {line}")
        patch, disposition, active_queue, provenance, regressions, rationale = columns
        provenance_rows[patch] = {
            "expected_disposition": disposition,
            "active_queue": active_queue == "yes",
            "provenance": [item for item in provenance.split("<br>") if item],
            "regressions": [item for item in regressions.split("<br>") if item],
            "rationale": rationale,
        }
    ...
    for patch in series_lines(VENDOR_ORIGINAL / "debian" / "patches" / "series"):
        ...
        metadata = provenance_rows.get(patch)
        if metadata is None:
            raise ValueError(f"Debian patch provenance document is missing {patch}")
        entries.append(
            {
                "patch": patch,
                "category": category,
...
def build_link_compat(records: list[dict[str, object]]) -> None:
    ...
    ensure_dir(abi_dir)
    baseline_abi_dir = Path(__file__).resolve().parents[1] / "abi" / "link-compat"
        ...
        for source_info in target.get("target_sources", []):
            ...
            if "linker" in source_info:
                ...
                if target.get("filename"):
                    objects = linker_objects.get(replace_for_replay(target["filename"][0]), [])
                    if objects:
    ...
    for phase, entry_ids in selections.items():
        baseline_path = baseline_abi_dir / f"{phase}.json"
        if baseline_path.exists():
            baseline_entry_ids = read_json(baseline_path).get("entry_ids", [])
            if set(baseline_entry_ids) == set(entry_ids):
                ordering = {entry_id: index for index, entry_id in enumerate(baseline_entry_ids)}
                entry_ids = sorted(entry_ids, key=lambda item: ordering[item])
