...
def build_packages(source_dir: Path, work_root: Path, label: str, profiles: str | None) -> dict[str, Path]:
    ...
    try:
        relative_work_root = work_root.resolve().relative_to(source_dir.resolve())
    except ValueError:
    ...
    if profiles:
        ...
        env_updates["DEB_BUILD_PROFILES"] = ""
        env_updates["SAFE_FULL_PACKAGE_BUILD"] = "1"
...
def generate_verify_set(source_dir: Path, work_root: Path, abi_shell_profiles: str) -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    shell_packages = build_packages(source_dir, work_root, "shell", abi_shell_profiles)
    abi_shell = manifest_from_packages("abi-shell", shell_packages, ABI_SHELL_PACKAGES & shell_packages.keys())
    runtime = manifest_from_packages("runtime", shell_packages, RUNTIME_PACKAGES)
    runtime_state = install_and_purge_runtime_state(
        {name: shell_packages[name] for name in sorted(RUNTIME_PACKAGES)},
        work_root,
    )
    return abi_shell, runtime, runtime_state
...
def verify_bootstrap(manifest_dir: Path, postinst_manifest: Path, generated: tuple[dict[str, object], ...]) -> None:
    abi_shell, runtime, runtime_state = generated
    ...
    # The richer package manifests are frozen bootstrap inputs in this phase.
    # They remain authoritative, but the ABI-shell verifier does not attempt to
    # rebuild docs, installed tests, GIR payloads, or udebs from scratch.
    read_json(manifest_dir / "dev-package.json")
    read_json(manifest_dir / "doc-package.json")
    read_json(manifest_dir / "udeb.json")
    read_json(manifest_dir / "full.json")
...
def main() -> None:
    ...
    ) if args.write_manifests else generate_verify_set(
        args.source.resolve(),
        args.work_root.resolve(),
        args.abi_shell_profiles,
    )
    if args.write_manifests:
        ...
        verify_bootstrap(args.install_manifests.resolve(), args.postinst_manifest.resolve(), generated)
