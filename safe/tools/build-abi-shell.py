...
def render_pc_file(name: str, build_root: Path, multiarch: str) -> str:
    ...
    libs = {
        "glib-2.0": "-lglib-2.0",
        "gthread-2.0": "-lgthread-2.0 -pthread",
        "gmodule-2.0": "-Wl,--export-dynamic",
        "gmodule-export-2.0": "-Wl,--export-dynamic",
        "gmodule-no-export-2.0": "-lgmodule-2.0 -pthread",
    ...
    requires = {
        ...
        "gmodule-2.0": "Requires: gmodule-no-export-2.0, glib-2.0\n",
        "gmodule-export-2.0": "Requires: gmodule-no-export-2.0, glib-2.0\n",
        ...
        "girepository-2.0": "Requires: glib-2.0, gobject-2.0\n",
    }[name]
    requires_private = {
        "glib-2.0": "Requires.private: libpcre2-8 >= 10.32\n",
        "gthread-2.0": "",
        "gmodule-2.0": "",
        "gmodule-export-2.0": "",
        "gmodule-no-export-2.0": "",
        "gobject-2.0": "Requires.private: libffi >= 3.0.0\n",
        "gio-2.0": "Requires.private: gmodule-no-export-2.0, zlib, mount >= 2.23, libselinux >= 2.2\n",
        "gio-unix-2.0": "",
    ...
    cflags = {
        "glib-2.0": common_cflags,
        "gthread-2.0": f"{common_cflags} -pthread",
        "gmodule-2.0": common_cflags,
        "gmodule-export-2.0": common_cflags,
        "gmodule-no-export-2.0": f"{common_cflags} -pthread",
        "gobject-2.0": common_cflags,
        "gio-2.0": common_cflags,
        "gio-unix-2.0": common_cflags,
        "girepository-2.0": common_cflags,
    }[name]
    variables = ""
    if name == "glib-2.0":
        variables = "\n".join(
    ...
    libs_line = f"Libs: -L${{libdir}} {libs}\n" if libs else ""
    return (
        f"prefix={SAFE_ROOT}\n"
        f"libdir={libdir}\n"
