...
LAYOUT_SEEDS = {
    ...
    "gobject": [
        ...
        ("gobject/gvaluearray.h", ["GValueArray"]),
        ("gobject/gobject.h", ["GObject", "GObjectClass"]),
        ("gobject/gparam.h", ["GParamSpec", "GParamSpecClass", "GParamSpecTypeInfo"]),
        ("gobject/gsignal.h", ["GSignalInvocationHint", "GSignalQuery"]),
        ("gobject/gclosure.h", ["GClosureNotifyData", "GClosure", "GCClosure"]),
...
def build_manifests() -> dict[str, dict[str, object]]:
    ...
    for library, headers in LAYOUT_SEEDS.items():
        ...
        for header_relpath, type_names in header_items:
