...
pub unsafe extern "C" fn canonicalize_filename(
    ...
    {
        if filename.is_null() {
            return legacy::canonicalize_filename(filename, relative_to);
        }
        ...
        let base = if is_absolute(filename) {
            ...
            match std::env::current_dir() {
                ...
                Err(_) => return legacy::canonicalize_filename(filename.as_ptr().cast(), relative_to),
            }
        } else {
            let relative_to = std::ffi::CStr::from_ptr(relative_to.cast()).to_bytes();
            if !is_absolute(relative_to) {
                return legacy::canonicalize_filename(filename.as_ptr().cast(), relative_to.as_ptr().cast());
            }
            relative_to.to_vec()
        };
    ...
    #[cfg(not(unix))]
    {
        legacy::canonicalize_filename(filename, relative_to)
    }
}
