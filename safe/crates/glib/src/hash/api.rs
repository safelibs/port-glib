...
pub unsafe extern "C" fn hash_table_new_similar(other_hash_table: *mut GHashTable) -> *mut GHashTable {
    if !is_owned_table(other_hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_NEW_SIMILAR,
            "g_hash_table_new_similar",
            unsafe extern "C" fn(*mut GHashTable) -> *mut GHashTable,
            other_hash_table,
        );
    }
    make_owned_table(
...
pub unsafe extern "C" fn hash_table_destroy(hash_table: *mut GHashTable) {
    if !is_owned_table(hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_DESTROY,
            "g_hash_table_destroy",
            unsafe extern "C" fn(*mut GHashTable),
            hash_table,
        );
        return;
    }
...
pub unsafe extern "C" fn hash_table_insert(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_INSERT,
            "g_hash_table_insert",
            unsafe extern "C" fn(*mut GHashTable, gpointer, gpointer) -> gboolean,
            hash_table,
            key,
            value,
        );
...
pub unsafe extern "C" fn hash_table_add(hash_table: *mut GHashTable, key: gpointer) -> gboolean {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_ADD,
            "g_hash_table_add",
            unsafe extern "C" fn(*mut GHashTable, gpointer) -> gboolean,
            hash_table,
            key,
        );
    }
...
pub unsafe extern "C" fn hash_table_remove(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_REMOVE,
            "g_hash_table_remove",
            unsafe extern "C" fn(*mut GHashTable, gconstpointer) -> gboolean,
            hash_table,
            key,
        );
    }
...
pub unsafe extern "C" fn hash_table_remove_all(hash_table: *mut GHashTable) {
    if !is_owned_table(hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_REMOVE_ALL,
...
pub unsafe extern "C" fn hash_table_steal_extended(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_STEAL_EXTENDED,
            "g_hash_table_steal_extended",
            unsafe extern "C" fn(*mut GHashTable, gconstpointer, *mut gpointer, *mut gpointer) -> gboolean,
            hash_table,
            lookup_key,
            stolen_key,
            stolen_value,
...
pub unsafe extern "C" fn hash_table_steal_all_keys(hash_table: *mut GHashTable) -> *mut GPtrArray {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_STEAL_ALL_KEYS,
            "g_hash_table_steal_all_keys",
            unsafe extern "C" fn(*mut GHashTable) -> *mut GPtrArray,
            hash_table,
        );
    }
    let entries = clear_table_raw(hash_table);
...
pub unsafe extern "C" fn hash_table_steal_all_values(hash_table: *mut GHashTable) -> *mut GPtrArray {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_STEAL_ALL_VALUES,
...
pub unsafe extern "C" fn hash_table_lookup(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_LOOKUP,
            "g_hash_table_lookup",
            unsafe extern "C" fn(*mut GHashTable, gconstpointer) -> gpointer,
            hash_table,
            key,
        );
    }
...
pub unsafe extern "C" fn hash_table_lookup_extended(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_LOOKUP_EXTENDED,
...
pub unsafe extern "C" fn hash_table_foreach(
    ...
    if !is_owned_table(hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_FOREACH,
            "g_hash_table_foreach",
            unsafe extern "C" fn(*mut GHashTable, GHFunc, gpointer),
            hash_table,
            func,
            user_data,
        );
...
pub unsafe extern "C" fn hash_table_find(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_FIND,
            "g_hash_table_find",
            unsafe extern "C" fn(*mut GHashTable, GHRFunc, gpointer) -> gpointer,
            hash_table,
            predicate,
            user_data,
        );
...
pub unsafe extern "C" fn hash_table_foreach_remove(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_FOREACH_REMOVE,
            "g_hash_table_foreach_remove",
            unsafe extern "C" fn(*mut GHashTable, GHRFunc, gpointer) -> guint,
            hash_table,
            func,
            user_data,
        );
...
pub unsafe extern "C" fn hash_table_foreach_steal(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_FOREACH_STEAL,
...
pub unsafe extern "C" fn hash_table_size(hash_table: *mut GHashTable) -> guint {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_SIZE,
            "g_hash_table_size",
            unsafe extern "C" fn(*mut GHashTable) -> guint,
            hash_table,
        );
    }
    (*hash_table).nnodes
...
pub unsafe extern "C" fn hash_table_get_keys(hash_table: *mut GHashTable) -> *mut GList {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_GET_KEYS,
            "g_hash_table_get_keys",
            unsafe extern "C" fn(*mut GHashTable) -> *mut GList,
            hash_table,
        );
    }
...
pub unsafe extern "C" fn hash_table_get_values(hash_table: *mut GHashTable) -> *mut GList {
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_GET_VALUES,
...
pub unsafe extern "C" fn hash_table_get_keys_as_array(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_GET_KEYS_AS_ARRAY,
            "g_hash_table_get_keys_as_array",
            unsafe extern "C" fn(*mut GHashTable, *mut guint) -> *mut gpointer,
            hash_table,
            length,
        );
    }
...
pub unsafe extern "C" fn hash_table_get_keys_as_ptr_array(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_GET_KEYS_AS_PTR_ARRAY,
...
pub unsafe extern "C" fn hash_table_get_values_as_ptr_array(
    ...
    if !is_owned_table(hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_GET_VALUES_AS_PTR_ARRAY,
...
pub unsafe extern "C" fn hash_table_iter_init(iter: *mut GHashTableIter, hash_table: *mut GHashTable) {
    if !is_owned_table(hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_INIT,
            "g_hash_table_iter_init",
            unsafe extern "C" fn(*mut GHashTableIter, *mut GHashTable),
            iter,
            hash_table,
        );
        return;
...
pub unsafe extern "C" fn hash_table_iter_next(
    ...
    if iter.is_null() || !is_owned_table((*iter).hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_NEXT,
            "g_hash_table_iter_next",
            unsafe extern "C" fn(*mut GHashTableIter, *mut gpointer, *mut gpointer) -> gboolean,
            iter,
            key,
            value,
        );
...
pub unsafe extern "C" fn hash_table_iter_get_hash_table(iter: *mut GHashTableIter) -> *mut GHashTable {
    if iter.is_null() || !is_owned_table((*iter).hash_table) {
        return oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_GET_HASH_TABLE,
...
pub unsafe extern "C" fn hash_table_iter_remove(iter: *mut GHashTableIter) {
    if iter.is_null() || !is_owned_table((*iter).hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_REMOVE,
            "g_hash_table_iter_remove",
            unsafe extern "C" fn(*mut GHashTableIter),
            iter,
        );
        return;
    }
...
pub unsafe extern "C" fn hash_table_iter_replace(iter: *mut GHashTableIter, value: gpointer) {
    if iter.is_null() || !is_owned_table((*iter).hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_REPLACE,
            "g_hash_table_iter_replace",
            unsafe extern "C" fn(*mut GHashTableIter, gpointer),
            iter,
            value,
        );
        return;
...
pub unsafe extern "C" fn hash_table_iter_steal(iter: *mut GHashTableIter) {
    if iter.is_null() || !is_owned_table((*iter).hash_table) {
        oracle_call!(
            ORACLE_G_HASH_TABLE_ITER_STEAL,
