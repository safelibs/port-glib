...
fn write_forwarders(out_dir: &Path, exports: &[Export]) {
    ...
    for export in exports {
        match export.kind {
            ExportKind::Function => {
                let slot = format!("__safe_gthread_slot_{}", export.ident);
                let _ = writeln!(
                    body,
    ...
    fs::write(out_dir.join("compat-forwarders.rs"), body)
...
fn main() {
    ...
    let original = manifest_dir
        .join("../../vendor/build-check/gthread/libgthread-2.0.so.0.8000.0")
        .canonicalize()
        .expect("missing frozen original gthread");
