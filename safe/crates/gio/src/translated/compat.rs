...
// SAFETY: Callers must pass a valid, properly aligned address for a 32-bit or 64-bit value.
pub unsafe fn atomic_load_acquire<T: Copy>(ptr: *mut T) -> T {
    match size_of::<T>() {
        4 => from_u32((*(ptr as *const AtomicU32)).load(Ordering::Acquire)),
        8 => from_u64((*(ptr as *const AtomicU64)).load(Ordering::Acquire)),
        size => panic!("unsupported atomic load size {size}"),
    }
}
...
// SAFETY: Callers must pass a valid, properly aligned address for a 32-bit or 64-bit integer.
pub unsafe fn atomic_xor_seqcst<T: Copy>(ptr: *mut T, value: T) -> T {
    match size_of::<T>() {
        4 => from_u32((*(ptr as *const AtomicU32).cast_mut()).fetch_xor(to_u32(value), Ordering::SeqCst)),
        8 => from_u64((*(ptr as *const AtomicU64).cast_mut()).fetch_xor(to_u64(value), Ordering::SeqCst)),
        size => panic!("unsupported atomic xor size {size}"),
    }
}

pub unsafe fn atomic_store_release<T: Copy>(ptr: *mut T, value: T) {
    match size_of::<T>() {
        4 => (*(ptr as *const AtomicU32).cast_mut()).store(to_u32(value), Ordering::Release),
        8 => (*(ptr as *const AtomicU64).cast_mut()).store(to_u64(value), Ordering::Release),
        size => panic!("unsupported atomic release store size {size}"),
    }
}

pub unsafe fn atomic_xchg_acquire<T: Copy>(ptr: *mut T, value: T) -> T {
    match size_of::<T>() {
        4 => from_u32((*(ptr as *const AtomicU32).cast_mut()).swap(to_u32(value), Ordering::Acquire)),
        8 => from_u64((*(ptr as *const AtomicU64).cast_mut()).swap(to_u64(value), Ordering::Acquire)),
        size => panic!("unsupported atomic acquire exchange size {size}"),
    }
}

pub unsafe fn atomic_xchg_release<T: Copy>(ptr: *mut T, value: T) -> T {
    match size_of::<T>() {
        4 => from_u32((*(ptr as *const AtomicU32).cast_mut()).swap(to_u32(value), Ordering::Release)),
        8 => from_u64((*(ptr as *const AtomicU64).cast_mut()).swap(to_u64(value), Ordering::Release)),
        size => panic!("unsupported atomic release exchange size {size}"),
    }
}

pub unsafe fn atomic_cxchg_acquire_relaxed<T: Copy>(ptr: *mut T, old: T, new: T) -> (T, bool) {
    match size_of::<T>() {
        4 => match (*(ptr as *const AtomicU32).cast_mut()).compare_exchange(
            to_u32(old), to_u32(new), Ordering::Acquire, Ordering::Relaxed,
        ) { Ok(previous) => (from_u32(previous), true), Err(previous) => (from_u32(previous), false) },
        8 => match (*(ptr as *const AtomicU64).cast_mut()).compare_exchange(
            to_u64(old), to_u64(new), Ordering::Acquire, Ordering::Relaxed,
        ) { Ok(previous) => (from_u64(previous), true), Err(previous) => (from_u64(previous), false) },
        size => panic!("unsupported atomic acquire compare-exchange size {size}"),
    }
}
